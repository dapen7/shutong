...
layui.define(["jquery","laytpl","layer"], function (exports) {
    ...
    Cascader.prototype={
        ...
        // 初始化容器和标签
        init: function () {
            $(this.elem).after('<span class="layui-edge" data-cascader="icon"></span>');
        ...
        finishInitData: function (triggerData) {
            ...
            $(this.elem).siblings("span").replaceWith('<span class="layui-edge" data-cascader="icon"></span>');
        ...
        callback: function () {
            ...
            // input点击显示隐藏
            $(document).on("click",self.elem+","+self.elem+" ~ span.layui-icon", function () {
                self.onOff = !self.onOff;
                zIndex++;
                if (self.onOff) {
                    $(self.elem).siblings(".urp-cascader-content").find("ul").slideDown(100);
                    $(self.elem).siblings("span").replaceWith('<span class="layui-edge-down" data-cascader="icon"></span>');

                    self.domContent.css("zIndex",zIndex);
                } else {
                    $(self.elem).siblings(".urp-cascader-content").find("ul").slideUp(100);
                    $(self.elem).siblings("span").replaceWith('<span class="layui-edge" data-cascader="icon"></span>');
                }
            })
            // 点击外层文档隐藏
